...
const service = {
  ...
  loads: () => FIND_MANY_RESULT,
  load: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Load", () => {
  ...
  test("GET /loads/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/loads"}/${nonExistingId}`)
  ...
  test("GET /loads/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/loads"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /loads existing resource", async () => {
